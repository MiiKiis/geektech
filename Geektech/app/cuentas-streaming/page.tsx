...
function StreamingContent() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const res = await fetch('/api/products/cuentas-streaming');
    ...
    const uniqueDurations = Array.from(new Set(products.map(p => p.duracion))).filter(Boolean);

    const filteredProducts = products.filter(p => {
        if (filterDuration !== 'all' && p.duracion !== filterDuration) return false;
        if (searchQuery.trim() !== '') {
            const q = searchQuery.toLowerCase();
            return p.plataforma?.toLowerCase().includes(q) || p.nombre?.toLowerCase().includes(q) || p.duracion?.toLowerCase().includes(q);
        }
        return true;
    });
            ...
            <div style={{ marginBottom: '32px', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '24px' }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <h1 style={{ fontSize: '1.875rem', fontWeight: 700, color: '#fff', borderLeft: '4px solid #22c55e', paddingLeft: '16px' }}>
                            Cuentas Streaming
                        </h1>
                        <span style={{
                            fontSize: '12px', fontWeight: 600, padding: '4px 12px', borderRadius: '9999px',
                            background: 'rgba(34,197,94,0.1)', color: '#4ade80', border: '1px solid rgba(255,255,255,0.05)'
                        }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div style={{
                                    ...
                                    {isSearching ? (
                                        <div style={{ padding: '24px', textAlign: 'center' }}>
                                            <div style={{ width: '28px', height: '28px', border: '3px solid rgba(34,197,94,0.3)', borderTop: '3px solid #22c55e', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 8px' }} />
                                                    ...
                                                    {items.map((item) => (
                                                        <button key={item.id} onClick={() => handleResultClick(item)}
                                                            ...
                                                            {item.price !== null && item.price > 0 && (
                                                                <span style={{ color: '#22c55e', fontSize: '14px', fontWeight: 700, flexShrink: 0 }}>Bs {item.price.toFixed(2)}</span>
                                                            )}
                                                        </button>
                                                    ))}
                                                </div>
                        ...
                        {uniqueDurations.length > 0 && (
                            <select value={filterDuration} onChange={(e) => setFilterDuration(e.target.value)}
                                ...
                                <option value="all">Todos — Duración</option>
                                {uniqueDurations.map((d: string) => <option key={d} value={d}>{d}</option>)}
                            </select>
                        )}
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center min-h-[40vh] w-full text-center">
                        <div className="bg-[#1e1e24] p-8 rounded-2xl border border-white/5 flex flex-col items-center max-w-md">
                            ...
                            <p className="text-gray-400">No hay cuentas que coincidan con tu búsqueda.</p>
                        </div>
                    </div>
                )}
            </main>
...
export default function StreamingPage() {
    return (
        <Suspense fallback={<div className="flex justify-center py-20"><div className="w-12 h-12 border-4 border-green-500 border-t-transparent rounded-full animate-spin" /></div>}>
            <StreamingContent />
        </Suspense>
    );
}
